...
void server::stop() {
    ...
    if (m_tcp_listen_fd != -1) {
        if (-1 ==
            epoll_ctl(m_epoll_fd, EPOLL_CTL_DEL, m_tcp_listen_fd, nullptr)) {
...
int server::tcp_socket_start() {
    ...
    if (-1 == bind(m_tcp_listen_fd, (struct sockaddr*)&server_addr,
                   sizeof(server_addr))) {
        PRINTF_ERROR("bind(%d, %s) error", m_tcp_listen_fd,
...
int server::udp_socket_start() {
    ...
    if (-1 ==
        bind(m_udp_fd, (struct sockaddr*)&server_addr, sizeof(server_addr))) {
        PRINTF_ERROR("bind(%d, %s) error", m_udp_fd,
                     inet_ntoa(server_addr.sin_addr));
        close(m_udp_fd);
        m_udp_fd = -1;
        return -1;
...
void server::do_udp_sendto(int fd, const char* data, int len,
                           struct sockaddr_in addr) {
    int total_send = 0;
    socklen_t addr_len = sizeof(addr);
    while (total_send < len) {
