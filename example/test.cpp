...
int get_batch_no(string& batch_no) {
  ...
  string post_string("");
  int ret = http_proc(url, 10, &vecHeadInfo, post_string, rsp_string, err_code,
                      err_msg);
  if (ret != 0) {
...
int cancel_redeem() {
  ...
  // PRINTF_DEBUG("post_string:[%s]", post_string.c_str());
  string url =
      "https://qc3.qwikcilver.com/QwikCilver/eGMS.RestAPI/api/gc/cancelredeem";
  vector<string> vecHeadInfo = g_vecHeadInfo;
  string transaction_id = string("TransactionId: ") + to_string(get_time_sec());
  vecHeadInfo.push_back(transaction_id);
  // string batch_no;
...
int reverse_redeem() {
  ...
  // PRINTF_DEBUG("post_string:[%s]", post_string.c_str());
  string url =
      "https://qc3.qwikcilver.com/QwikCilver/eGMS.RestAPI/api/gc/reverseredeem";
  vector<string> vecHeadInfo = g_vecHeadInfo;
  string transaction_id = string("TransactionId: ") + order_id;
  vecHeadInfo.push_back(transaction_id);
  // string batch_no;
