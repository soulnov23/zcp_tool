...
int do_listen() {
    ...
    if (-1 == make_socket_nonblocking(listen_fd)) {
        LOG_ERROR("make_socket_nonblocking({}) error", listen_fd);
        return -1;
    }
    ...
    if (-1 == epoll_ctl(epoll_fd, EPOLL_CTL_ADD, listen_fd, &event)) {
        LOG_ERROR("epoll_ctl({}, EPOLL_CTL_ADD, {}) error", epoll_fd, listen_fd);
        return -1;
    }
    if (-1 == make_socket_reuseaddr(listen_fd)) {
        LOG_ERROR("make_socket_reuseaddr({}) error", listen_fd);
        return -1;
    }
    if (-1 == bind(listen_fd, (struct sockaddr*)&server_addr, sizeof(server_addr))) {
        LOG_ERROR("bind({}, {}) error", listen_fd, inet_ntoa(server_addr.sin_addr));
        close(listen_fd);
        listen_fd = -1;
        return -1;
    }
    if (-1 == listen(listen_fd, 128)) {
        LOG_ERROR("listen({}, 128) error", listen_fd);
        close(listen_fd);
        listen_fd = -1;
        return -1;
    }
...
void do_accept() {
    while (true) {
        ...
        if (-1 == epoll_ctl(epoll_fd, EPOLL_CTL_ADD, fd, &event)) {
            LOG_ERROR("epoll_ctl({}, EPOLL_CTL_ADD, {}) error", epoll_fd, fd);
        }
    }
}
        ...
        if (ret > 0) {
            ...
            if ((errno == EAGAIN) || (errno == EWOULDBLOCK)) {
                ...
                if (-1 == epoll_ctl(epoll_fd, EPOLL_CTL_DEL, fd, nullptr)) {
                    LOG_ERROR("epoll_ctl({}, EPOLL_CTL_DEL, {}) error", epoll_fd, fd);
                }
                close(fd);
                break;
            }
...
int main(int argc, char* argv[]) {
    LOG_DEBUG("example_epoll start success");
    LOG_DEBUG("push");
    // cout << "push" << endl;
    ...
    while (true) {
        ...
        if (-1 == count) {
            LOG_ERROR("epoll_wait({}) error", epoll_fd);
            return -1;
        }
        for (int i = 0; i < count; i++) {
            // LOG_DEBUG("fd:{} event:{}", &(events[i].data.fd), &(events[i].events));
