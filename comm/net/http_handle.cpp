...
int http_proc(const std::string& strUrl, unsigned iTimeout,
   ...
   * curl_global_init(3) yourself properly
  ...
  /*
     ...
     strDnsList.c_str());
          ...
          if (!strIpv4.empty()) {
                  curl_easy_setopt(curl, CURLOPT_DNS_LOCAL_IP4,
     strIpv4.c_str());
          }
  */
  ...
  /* comment, gerryyang
   * In unix-like systems, CURLOPT_TIMEOUT might cause signals to be used unless
   * CURLOPT_NOSIGNAL is set.
   * CURLOPT_NOSIGNAL, Pass a long. If it is 1, libcurl will not use any
   * functions that install signal handlers or any functions that cause signals
   * to be sent to the process. This option is mainly here to allow
   * multi-threaded unix applications to still set/use all timeout options etc,
   * without risking getting signals. The default value for this parameter is 0.
   * (Added in 7.10)
