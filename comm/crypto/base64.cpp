...
int base64_encode(const std::string& str_in, std::string& str_out) {
    ...
    for (; i + 3 <= str_in.length(); i += 3) {
        ...
        res.push_back(
            base64_encode_table[(((((unsigned char)str_in[i]) & 0x03) << 4) |
                                 (((unsigned char)str_in[i + 1]) >> 4))]);
        res.push_back(
            base64_encode_table[((((unsigned char)str_in[i + 1]) & 0x0F) << 2) |
                                (((unsigned char)str_in[i + 2]) >> 6)]);
        res.push_back(
            base64_encode_table[((unsigned char)str_in[i + 2]) & 0x3F]);
    }
    if (i + 1 == str_in.length()) {
        ...
        res.push_back(
            base64_encode_table[(((((unsigned char)str_in[i]) & 0x03) << 4) |
                                 (((unsigned char)str_in[i + 1]) >> 4))]);
        res.push_back(base64_encode_table
                          [((((unsigned char)str_in[i + 1]) & 0x0F) << 2)]);
        res.push_back('=');
    }
    str_out = res;
    return 0;
...
int base64_decode(const std::string& str_in, std::string& str_out) {
    ...
    std::string::size_type last_eq = str_in.find('=');
    if (last_eq != std::string::npos) {
        if (last_eq + 2 < str_in.length()) {
            return -1;
        } else if (last_eq + 2 == str_in.length() &&
                   str_in[last_eq + 1] != '=') {
            return -1;
        }
    }
