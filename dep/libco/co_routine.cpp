...
void OnEventProcess( stTimeoutItem_t * ap )
{
	...
	if (lp->persist && lp->timeout > 0)  
	{
		unsigned long long now = GetTickMS();
		lp->ullExpireTime = now + lp->timeout;
		int ret = AddTimeout( ctx->pTimeout, lp, now );
		if( ret != 0 )
		{
			epoll_ctl(epfd, EPOLL_CTL_DEL, lp->fd, &lp->stEvent);
			co_log_err("CO_ERR: AddTimeout ret %d now %lld timeout %d arg.ullExpireTime %lld",
					ret, now, lp->timeout, lp->ullExpireTime);
			errno = EINVAL;
		}
	}
}
...
{
	stCoEvent_t* co_event = (stCoEvent_t*)calloc(1, sizeof(stCoEvent_t));
	return co_event;
}

int co_add_event(stCoEvent_t* lp, int fd, pfn_co_event_call_back pfn, void* args, 
		unsigned int events, int timeout, bool persist )
{
	//init
	lp->fd = fd;
	lp->pfn = pfn;
	...
	else if (lp->stEvent.events == 0)
	{
		lp->stEvent.events = PollEvent2Epoll(events);
		lp->stEvent.data.ptr = (void*)lp;
		ret = epoll_ctl(epfd, EPOLL_CTL_ADD, lp->fd, &lp->stEvent);
	}
	else if (lp->stEvent.events != events)
	{
		lp->stEvent.events = PollEvent2Epoll(events);
		lp->stEvent.data.ptr = (void*)lp;
...
stCoEvent_t* co_new_timer(pfn_co_event_call_back pfn, void* args, int timeout, bool persist)
{
	stCoEvent_t* event = co_alloc_event();
	int ret = co_add_event(event, -1, pfn, args, 0, timeout, persist);
	if (ret) 
	{
		co_free_event(event);
		return NULL;
