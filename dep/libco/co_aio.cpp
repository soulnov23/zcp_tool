...
class clsCoAio {
 ...
 public:
  ...
  struct stStat {
    ...
    stStat() { memset(this, 0, sizeof(*this)); }
 ...
 public:
  explicit clsCoAio(int event_cnt = kNumEventPerThread)
      : eventfd_(-1), first_submit_time_(0), submit_signal_(0), error_(0) {
    // io_setup
  ...
  const stStat &Stat() const { return stat_; }
 ...
 public:
  static ssize_t co_pread(int fd, void *buf, size_t count, off_t offset,
                          clsCoAio *o) {
 ...
 public:
  ...
  static ssize_t co_pwrite(int fildes, const void *buf, size_t nbyte,
                           off_t offset, clsCoAio *aio) {
    struct cb_t cb = {0};
    cb.ret_code = -1;
    cb.complete_bytes = 0;
  ...
  inline ssize_t align_pread(int fildes, void *buf, size_t nbyte,
                             off_t offset) {
    struct cb_t cb = {0};
    cb.ret_code = -1;
    cb.complete_bytes = 0;
...
void co_set_force_pread2(bool b) { s_force_pread2 = b; }
...
int co_fsync(int fildes) { return clsCoAio::co_fsync(fildes, o); }
int co_aio_init_ct() {
  if (!o) {
    o = new clsCoAio();
    o->start_tick_co();
...
class clsCoPreadThr {
 ...
 public:
  clsCoPreadThr() {
    ...
    if (ret != 0) {
      co_log_err("%s:%d setnonblock ret %d errno %d", __func__, __LINE__, ret,
                 errno);
    }
  }
  ~clsCoPreadThr() { delete input_; }
...
static char gzrd_Lib_CPP_Version_ID[] __attribute__((used)) =
    ...
    "colib/co_aio.cpp $ $Id: co_aio.cpp 2799808 2018-10-24 06:25:10Z leiffyli "
    "$ " GZRD_SVN_ATTR "__file__";
