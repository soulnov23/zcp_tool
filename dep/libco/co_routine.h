...
int co_add_event(stCoEvent_t* coevent, int fd, pfn_co_event_call_back pfn, void* args, 
		unsigned int events, int timeout, bool persist = false);
int co_free_event(stCoEvent_t* co_event);

//5.timeout
stCoEvent_t* co_new_timer(pfn_co_event_call_back pfn, void* args, int timeout, bool persist = false);
